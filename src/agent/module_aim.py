...
from configs.simulation import MAXIMUM_HAND_SPEED, BUMP_INTERVAL, MUSCLE_INTERVAL, MAXIMUM_CAM_SPEED
from utils.mymath import target_game_position, cart2sphr, replicate_target_movement
...
def plan_hand_movement(
    ...
    cpos,   # Current state
    sensi,  # Current state
    plan_duration,                      # Unit: msec
    execute_duration,                   # Unit: msec
    interval=MUSCLE_INTERVAL,           # Unit: msec
    maximum_camera_speed=MAXIMUM_CAM_SPEED     # Unit: deg/s
):
    '''
    ...
    tgpos = target_game_position(ppos, pcam, tmpos)
    cgpos = target_game_position(ppos, pcam, cpos)

    # Compute required cam adjustment
    cam_adjustment = cart2sphr(*tgpos) - cart2sphr(*cgpos)
    ...
    maximum_hspd = maximum_camera_speed / sensi
...
def implicit_aim_point(r):
    a = np.random.uniform(-np.pi, np.pi)
    rr = np.random.uniform(0, r)
    return np.array([np.cos(a), np.sin(a)]) * rr
...
if __name__ == "__main__":
    ...
    pn, vn = add_motor_noise(p[0], v, 0.15)
    pp, vv = interpolate_plan(pn, vn, 50, 1)

    plt.scatter(*pp.T, s=0.1, color='k')
    plt.scatter(*pn.T, s=0.1, color='r')
    plt.show()

    g.move_hand(pn[-1] - pn[0])
    g.show_monitor()
